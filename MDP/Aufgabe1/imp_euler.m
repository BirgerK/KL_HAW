...
lasty = y0;
    ...
    for x=firstx:h:xend
        options = optimset('Display','off');                    %Einstellen der Optionen f�r das f Solve auf keine Ausgabe
        yn1appro = fsolve(@(n)n-lasty-h*f0(x, n), x , options); % Zuweisung des y wertes auf n und definition innerhalb der Funktion
        yn1 = lasty + h * f0(x,yn1appro);                       %L�sen der Gleichung mit Approximiertem Y
